...
#             + os.path.join(self.app.property("ResPath"), "www", self.html).replace('\\', '/')
        ...
        self.icon = self._getQIcon('evo_64x64.png')
...
class MainWebUI(BaseWebUI):
    def __init__(self, app, hub, debug):
        ...
        if sys.platform == 'darwin':
            tray_icon = 'evo_16x16_mac.png'
        elif sys.platform == "win32":
            tray_icon = 'evo_16x16.png'
        else:
            tray_icon = 'evo_32x32_ubuntu.png'
        ...
        self.cryptonoted_daemon_manager = None
...
#         self.view.load(qt_core.QUrl(self.url))
    ...
    def run_wallet_rpc(self, wallet_password, log_level=1):
        ...
        while True:
            ...
            cryptonoted_info = self.daemon_rpc_request.get_info()
            if cryptonoted_info['status'] == "OK":
    ...
    def _update_daemon_status(self):
        ...
        if cryptonoted_info['status'] == "OK":
            status = "Connected"
            self.current_height = int(cryptonoted_info['height'])
            target_height = int(cryptonoted_info['target_height'])
            ...
            status = cryptonoted_info['status']
        ...
        sync_status = "Disconnected" if cryptonoted_info['status'] != "OK" else "Synchronizing..."
        if cryptonoted_info['status'] == "OK" and self.current_height == self.target_height:
...
#             print(json.dumps(wallet_info, indent=4))
    ...
    def about(self):
        QMessageBox.about(self, "About", \
            u"%s <br><br>Copyright© 2018 - CryptonoteEvo Projects (www.nur1labs.net)" % self.agent)
