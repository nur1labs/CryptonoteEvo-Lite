...
'''
Process managers for cryptonoted, avo-wallet-cli and avo-wallet-rpc
'''
...
class WalletCliManager(ProcessManager):
    ...
    def __init__(self, resources_path, wallet_file_path, wallet_log_path, restore_wallet=False, restore_height=0):
        if not restore_wallet:
            wallet_args = u'%s/bin/evo-wallet-cli --daemon-address %s --generate-new-wallet=%s --log-file=%s ' \
                                                % (resources_path, REMOTE_DAEMON_ADDRESS, wallet_file_path, wallet_log_path)
        else:
            wallet_args = u'%s/bin/evo-wallet-cli --daemon-address %s --log-file=%s --restore-deterministic-wallet --restore-height %d' \
                                                % (resources_path, "fakehost", wallet_log_path, restore_height)
        ProcessManager.__init__(self, wallet_args, "evo-wallet-cli")
        self.ready = Event()
        self.last_error = ""
...
class WalletRPCManager(ProcessManager):
    def __init__(self, resources_path, wallet_file_path, wallet_password, app, log_level=1, enable_ssl=False):
        ...
        wallet_log_path = os.path.join(os.path.dirname(wallet_file_path), "evo-wallet-rpc.log")
        if enable_ssl:
            wallet_rpc_args = u'%s/bin/evo-wallet-rpc --daemon-address %s --wallet-file %s --log-file %s --rpc-bind-port %d --user-agent %s --log-level %d --enable-ssl --cacerts-path %s' \
            ...
            wallet_rpc_args = u'%s/bin/evo-wallet-rpc --daemon-address %s --wallet-file %s --log-file %s --rpc-bind-port %d --user-agent %s --log-level %d' \
                                            % (resources_path, REMOTE_DAEMON_ADDRESS, wallet_file_path, wallet_log_path, WALLET_RPC_PORT, self.user_agent, log_level)
        ProcessManager.__init__(self, wallet_rpc_args, "evo-wallet-rpc")
        sleep(0.2)
